...
const CategoryContent: React.FC<{ products: Product[], type: string }> =
    ({ products, type }: { products: Product[], type: string }) => {
        ...
        return (
            <>
                <main className="grid grid-cols-3 gap-5 my-6 tab:grid-cols-2 h-[117rem] tab:h-[157rem]">
                    {
                        displayedProducts?.map((pro: Product, index: number) => (
                            <Link href={`/category/${type}/${pro.title.replaceAll(' ', '-').toLowerCase()}`} key={index} className="relative h-[35rem]">
